...
### 2. 服务名称检查
- [ ] 是否首先执行了 `signoz_list_services` 确认服务名称
- [ ] 服务名称是否与实际运行时的名称匹配
- [ ] 是否使用了 `service_name_mapping` 映射
...
1. **优先使用 Query Builder v5**：`signoz_execute_builder_query` 更灵活，支持复杂过滤条件
2. **必须首先执行 list_services**：确认实际服务名称
...
8. **验证结果**：检查 `rows` 和 `warnings` 字段，确保查询成功
