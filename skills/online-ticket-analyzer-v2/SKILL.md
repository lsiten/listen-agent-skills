...
🚨🚨🚨 **严格约束：禁止在没有满足退出条件时进入下一步！** 🚨🚨🚨

**❌ 绝对禁止的行为**：
- ❌ 查询返回空结果后，直接进入阶段2进行分析
- ❌ 没有查询到相关日志，却开始分析代码并给出结论
- ❌ 自动调整策略还没穷尽，就开始生成分析报告
- ❌ 没有询问用户是否跳过或是否能提供更多信息，就直接进入分析

**✅ 正确的行为**：
- ✅ 查询无结果 → 尝试自动调整策略（放宽条件、扩展时间、切换时间优先级等）
- ✅ 自动调整后仍无结果 → 询问用户是否能提供更多信息
- ✅ 用户提供新信息 → 继续查询循环
- ✅ 用户明确表示无法提供更多信息或要求跳过 → 才能退出循环进入阶段2

**检查点：退出循环前必须确认**：
1. [ ] 是否查询到了相关日志？如果是 → 可以退出
2. [ ] 用户是否明确要求跳过？如果是 → 可以退出
3. [ ] 是否已询问用户且用户确认无法提供更多信息？如果是 → 可以退出
4. [ ] 以上都不是？→ **禁止退出循环！必须继续尝试或询问用户**
