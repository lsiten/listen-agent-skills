...
2. **执行初始MCP查询** - 按照`.online-ticket-analyzer/tickets/ticket_xxx/mcp_instructions.json`中的初始指令，调用SigNoz MCP工具
   ...
   - ⚠️ **关键**：`signoz_list_services` 默认时间范围是 6 小时，如果该时间段内服务没有数据，会返回空数组
   - ⚠️ **必须**：使用**更长的时间范围**（24h 或 7d）来确认服务名称，而不是默认的 6h
   ...
   - ⚠️ **查询结果为空（rows 为 []）的处理**：
     - **诊断步骤**（按顺序执行）：
       1. **验证服务名称**：执行`signoz_list_services`确认服务名称是否正确
          - ⚠️ **关键**：必须使用**更长的时间范围**（24h 或 7d），而不是默认的 6h
          - 如果 `signoz_list_services` 返回空数组，尝试更长的时间范围（7d 或 30d）
       2. **检查 rowsScanned 字段**：
          - 如果 `rowsScanned: 0`，说明该时间段确实没有该服务的数据
          - 可能原因：服务名称不匹配、该时间段服务确实没有产生日志、日志可能存储在其他服务名称下
          - 解决方案：扩展时间范围、切换到其他时间优先级、查询不限定用户ID的服务日志
       3. **逐步放宽查询条件**：
          - 先查询服务 + 时间范围（不限定用户/设备）
          - 如果有数据，再逐步添加过滤条件（用户ID → 错误级别 → 接口路径）
       3. **扩展时间范围**：如果当前时间范围无数据，尝试扩展时间范围（±2小时 → ±4小时 → ±24小时）
       4. **切换到其他时间优先级**：如果当前时间优先级无数据，切换到下一个时间优先级
       5. **检查字段值**：使用`signoz_get_logs_field_values`检查字段的实际值
     - **容错策略**：
       - 如果查询无结果，自动尝试放宽查询条件
       - 如果放宽条件后仍无结果，自动扩展时间范围
       - 如果扩展时间范围后仍无结果，切换到下一个时间优先级
   - ⚠️ **查询超时处理**：
     - 自动缩短时间范围（缩短为原来的一半）
     - 简化查询条件（移除不必要的过滤条件）
     - 使用 limit 限制返回记录数（默认 100，最大建议 1000）
     - 自动重试（最多重试 3 次）
