...
- 🌍 **普遍性问题分析** - **新增** - 分析问题是否可能是普遍性问题（某个国家/地区、某个环境一定会出现的），根据相应特征查询SigNoz数据进行分析，如果是普遍问题，生成的报告会特意标注
- 💡 **解决方案生成** - 综合日志、代码、历史经验和普遍性分析结果，生成问题原因和解决方案
...
- **发生时间**（可选，支持多种时间格式，详见下方时间查询优先级说明）
  - ⚠️ **重要**：如果输入是邮件沟通记录，可能包含多个时间点（多个邮件的发送时间）
  - 系统会自动提取多个时间，AI会分析确定关键时间范围
- **发送方信息**（可选，如果是邮件沟通记录）
  - ⚠️ **重要**：如果输入是邮件沟通记录，可能包含多个发送方
  - 系统会自动提取多个发送方，AI会分析识别主要发送方和关键参与者
...
2. **工单上报时间或邮件发送时间**：从问题描述中自动提取工单上报时间或邮件发送时间
   - ⚠️ **重要**：如果检测到多个时间点（邮件沟通记录），AI会分析确定关键时间范围
   - 系统会使用最早和最晚时间，查询该时间范围前后2小时的时间段
3. **问题发生时间**：从问题描述中提取的问题发生时间
   - ⚠️ **重要**：如果检测到多个问题发生时间，系统会使用最早和最晚时间
   - 查询该时间范围前后2小时的时间段
4. **最近1小时**：如果以上都没有，默认查询最近1小时

**多发送方和多时间处理**：
- ⚠️ **重要**：如果输入是邮件沟通记录，系统会自动检测多个发送方和多个时间
- 脚本会辅助提取这些信息，但真正的分析需要AI完成
- AI会分析邮件上下文，理解对话流程，识别关键时间点和发送方
- AI分析结果会保存到工单上下文中，用于后续查询和分析
...
3. **解析用户输入**：提取工单信息（服务名、时间、用户信息等）
   - ⚠️ **重要**：支持邮件沟通记录，可能包含多个发送方和多个时间
   - 脚本会辅助提取多个发送方和时间，但真正的分析需要AI完成
   - AI会分析邮件上下文，理解对话流程，识别关键时间点和发送方
4. **AI分析多发送方和多时间**：**新增功能** - 如果检测到多个发送方或多个时间，AI会分析：
   - 识别邮件沟通记录中的多个发送方
   - 识别多个时间点（邮件发送时间、问题发生时间等）
   - 理解邮件上下文和对话流程
   - 确定最相关的时间范围用于查询
   - 识别主要发送方和关键时间点
5. **计算查询时间范围**：根据优先级计算查询时间范围（支持多个时间点，使用最早和最晚时间）
6. **保存工单上下文**：保存到`.production-issue-analyzer/tickets/{ticket_id}/ticket_context.json`（包含多发送方和多时间信息）
7. **生成MCP调用指令**：生成到`.production-issue-analyzer/tickets/{ticket_id}/mcp_instructions.json`
...
2. **检查查询结果**：**新增功能** - 检查查询结果是否为空
   - 如果查询结果为空，生成初步判断文档（`preliminary_analysis.md`）
   - 提示用户提供更精确的信息（用户ID、设备ID、接口路径、时间范围等）
   - 分析可能的原因（时间范围、服务名称、字段歧义等）
   - 给出具体建议，由用户判断是否需要进一步确定
   - **不会生成完整解决方案**，而是等待用户提供更精确信息后重新分析
3. **处理日志数据**：深度分析日志，提取关键错误信息（不发送原始日志给大模型）
4. **提取公共信息**：更新工单上下文
5. **分析代码逻辑**：基于错误信息定位并分析相关代码文件
6. **检索历史经验**：从`.production-history/`目录检索相似问题的历史经验
7. **分析普遍性问题**：**新增功能** - 分析问题是否可能是普遍性问题（某个国家/地区、某个环境一定会出现的）
   - 提取关键特征（国家/地区、环境、服务版本、浏览器版本等）
   - 生成广泛查询（不限定用户ID、设备ID）
   - 查询SigNoz获取相关数据
   - AI分析判断是否是普遍性问题
   - 如果是普遍性问题，在解决方案中特别标注
8. **生成综合解决方案**：综合日志、代码、历史经验和普遍性分析结果，生成问题原因和解决方案（仅在查询结果不为空时）
9. **保存工单处理记录**：更新工单上下文
10. **保存经验**：将解决经验保存到`.production-history/experience_{hash}.md`（仅在查询结果不为空时）
11. **输出解决方案文档**：生成到`.production-issue-analyzer/tickets/{ticket_id}/solution.md`（如果是普遍性问题会特别标注，仅在查询结果不为空时）
...
└── tickets/                      # 工单目录
    └── {ticket_id}/              # 每个工单独立目录
        ...
        ├── preliminary_analysis.md       # 初步判断文档（阶段2生成，当查询结果为空时）
        └── solution.md           # 解决方案文档（阶段2生成，如果是普遍性问题会特别标注，仅在查询结果不为空时）
```
...
⚠️ **新增功能**：在分析完工单问题后，系统会自动分析是否可能是普遍性问题。

**分析流程**：

1. **特征提取**：从工单信息和日志分析中提取关键特征：
...
2. **生成广泛查询**：基于提取的特征，生成不限定用户ID和设备ID的广泛查询：
   - 扩展时间范围到前后24小时（获取更多样本）
   - 只使用特征条件过滤（服务、环境、版本、地理位置、浏览器等）
   - 不添加用户ID和设备ID过滤

3. **AI执行查询**：生成`prevalence_instructions.json`，等待AI执行查询并保存结果到`prevalence_results.json`

4. **分析结果**：系统分析查询结果，判断是否是普遍性问题：
   - **影响范围统计**：错误数量、用户数量、设备数量、国家/地区数量、城市数量
   - **普遍性级别判断**：
...
- **影响用户数**: 23
- **影响设备数**: 28
- **受影响国家/地区**: China, United States
- **受影响城市**: Shanghai, Beijing, New York

**建议**: ⚠️ **严重普遍性问题**：此问题影响多个国家/地区的大量用户，建议立即采取紧急措施，考虑回滚或发布热修复。
```
