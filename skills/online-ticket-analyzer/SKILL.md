...
6. **分析普遍性问题**：**新增功能** - 分析问题是否可能是普遍性问题（某个国家/地区、某个环境一定会出现的）
   - 提取关键特征（国家/地区、环境、服务版本、浏览器版本等）
   - 生成广泛查询（不限定用户ID、设备ID）
   - 查询SigNoz获取相关数据
   - AI分析判断是否是普遍性问题
   - 如果是普遍性问题，在解决方案中特别标注
7. **生成综合解决方案**：综合日志、代码、历史经验和普遍性分析结果，生成问题原因和解决方案
8. **保存工单处理记录**：更新工单上下文
9. **保存经验**：将解决经验保存到`.production-history/experience_{hash}.md`
10. **输出解决方案文档**：生成到`.production-issue-analyzer/tickets/{ticket_id}/solution.md`（如果是普遍性问题会特别标注）
